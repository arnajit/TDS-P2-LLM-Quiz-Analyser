...
from tools import (
    ...
    run_code, add_dependencies, ocr_image_tool, encode_image_to_base64, transcribe_audio
)
...
def agent_node(state: AgentState):
    ...
    # -------------------------------------------------
    # AUTO-CHAIN WHEN audio_url IS DETECTED
    # -------------------------------------------------
    try:
        last_content = result.content
        if isinstance(last_content, dict) and last_content.get("audio_url"):
            audio_url = last_content["audio_url"]
            print("Detected audio_url -> Forcing LLM to download and transcribe:", audio_url)

            force_msg = HumanMessage(content=f"""
            A rendered page contains an audio file: {audio_url}.
            You MUST now:
            1. Call download_file with this audio URL.
            2. Pass the resulting file path to transcribe_audio.
            3. Submit the transcript as the answer.
            DO NOT think. Immediately call download_file.
            """)

            forced = llm.invoke([force_msg])
